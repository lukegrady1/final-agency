...
// [ASSET NEEDED] Drop your 60–90s face-to-camera welcome video at
// public/welcome_video.mp4 and it will replace the placeholder automatically.
const WELCOME_VIDEO_SRC = "/welcome_video.mp4";
  ...
  return (
    <BlurIn>
      <div className="relative max-w-3xl mx-auto">
        {!isMuted ? null : (
          <span className="absolute -top-3 left-1/2 -translate-x-1/2 z-20 rounded-full bg-accent text-white text-xs font-medium px-3 py-1 shadow-lg">
            Click for sound
          </span>
        )}
        <div className="group relative aspect-video rounded-2xl overflow-hidden bg-card border border-card-border">
          {hasVideo ? (
            ...
            // Placeholder shown until welcome_video.mp4 is added.
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
              <div className="w-14 h-14 rounded-full bg-accent/15 border border-accent/30 flex items-center justify-center text-accent-light">
                <Video className="w-6 h-6" />
              </div>
              <p className="text-white/70 text-sm mt-4 max-w-xs">
                A quick welcome video from Luke is on the way. In the meantime,
                head to Step 2 below and run your free audit.
              </p>
            </div>
          )}
        </div>
