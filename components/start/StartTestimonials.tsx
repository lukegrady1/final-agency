  ...
  return (
    <StaggerParent className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {testimonials.map((t, i) => (
        <StaggerChild
          key={i}
          className="bg-white border border-black/[0.08] rounded-2xl shadow-sm p-6 flex flex-col"
        >
          <Quote className="w-6 h-6 text-accent/60 mb-4" />
          <p className="text-[#0c0b1e]/80 text-sm leading-relaxed flex-1">
            &ldquo;{t.quote}&rdquo;
          </p>
          ...
          <div className="mt-3 pt-4 border-t border-black/10">
            <div className="text-[#0c0b1e] text-sm font-medium">{t.name}</div>
            <div className="text-[#0c0b1e]/50 text-xs mt-0.5">{t.title}</div>
          </div>
        </StaggerChild>
      ))}
    </StaggerParent>
